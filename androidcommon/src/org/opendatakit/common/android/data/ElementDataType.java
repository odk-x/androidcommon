 ...
 * 
 * Note that because 'boolean' is a reserved word, we cannot 
 * use an enum (and we shorten that type to 'bool' here).
 * 
 * It provides the name() and a modified valueOfOrNull() that
 * acts as 

 * to shorten that enum to 'bool'. To preserve the use of 
 * 'boolean' in the transport, use the 'nameExternal()' and
 * 'valueOfExternalOrNull(...)' methods instead of the 'name()'
 * and 'valueOf(...)' methods.
 * 
