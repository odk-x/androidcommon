...
public abstract class WebKitDatabaseInfoHelper extends ODKSQLiteOpenHelper {
  ...
  public void onCreate(SQLiteDatabase db) {
    db.execSQL("CREATE TABLE IF NOT EXISTS " + WEBKIT_INFO_TABLE + " (" + WEBKIT_INFO_KEY
        + " text, " + WEBKIT_INFO_VALUE + " text );");
    ...
    db.execSQL("CREATE TABLE IF NOT EXISTS " + WEBKIT_APP_VERSION_TABLE + " (" + WEBKIT_APP_KEY
        + " text, " + WEBKIT_APP_VERSION + " integer );");
  }
  ...
  public abstract void onUpgradeAppVersion(SQLiteDatabase db, int oldVersion, int newVersion);
  ...
  private void assertAppVersion(SQLiteDatabase db, String appKey, int version) {
    ...
    Cursor c = null;
    try {
      c = db.query(WEBKIT_APP_VERSION_TABLE, null, WEBKIT_APP_KEY + "=?", new String[] { appKey },
          null, null, WEBKIT_APP_VERSION + " DESC");
    ...
    if (actualVersion == null || actualVersion != version) {
      ...
      try {
        ...
        db.delete(WEBKIT_APP_VERSION_TABLE, WEBKIT_APP_KEY + "=?", new String[] { appKey });
