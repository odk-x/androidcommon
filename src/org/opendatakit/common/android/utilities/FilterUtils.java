...
/**
 * Useful static methods to construct the selection string and selectionArgs
 * array for passing to a content provider. Handles the details of null-valued
 * criteria.
 *
...
public final class FilterUtils {
	...
	/**
	 * Builds the selection string and selectionArgs[] arrays for a
	 * ContentProvider or DB query. Takes an array of keys (Strings) and a
	 * corresponding array of values for those keys. Constructs the selection
	 * clause, allowing null values in the value list and properly representing
	 * those constraints as "key is null". Assumes the toString() method will
	 ...
	 * @return
	 */
	public static final FilterCriteria buildSelectionClause(String[] keys,
			Object[] values, String additionalClause) {
		if (keys.length != values.length)
			throw new IllegalArgumentException(
					"mismatched lengths for key and value arrays");
	...
	public static final FilterCriteria buildSelectionClause(String key,
			Object value) {
		StringBuilder b = new StringBuilder();
		String[] selectionArgs;
		if (value != null) {
	...
	public static final FilterCriteria buildInverseSelectionClause(String key,
			Object value) {
		StringBuilder b = new StringBuilder();
		String[] selectionArgs;
		if (value != null) {
	...
	public static final FilterCriteria buildAsGiven(String selection,
			String[] selectionArgs) {
		boolean nonBlankSelection = (selection != null && selection.trim()
				.length() != 0);
		if (nonBlankSelection && selectionArgs != null
	...
	public static final FilterCriteria buildAsGivenWithUri(String selection,
			String[] selectionArgs, String keyRowId, Uri uri) {

		if (uri.getPathSegments().size() < 2) {
			throw new IllegalArgumentException("Expected a rowId qualifier: "
					+ uri.toString());
		}
		String key = uri.getPathSegments().get(1);
		FilterUtils.FilterCriteria fGiven = FilterUtils.buildAsGiven(selection,
				selectionArgs);
		FilterUtils.FilterCriteria fKeyId = FilterUtils.buildSelectionClause(
				keyRowId, key);
		return FilterUtils.and(fGiven, fKeyId);
	}

	private static final FilterCriteria join(FilterCriteria a, String join,
			FilterCriteria b) {
		StringBuilder selection = new StringBuilder();
		boolean aNonBlankSelection = (a.selection != null && a.selection.trim()
				.length() != 0);
		boolean bNonBlankSelection = (b.selection != null && b.selection.trim()
				.length() != 0);
