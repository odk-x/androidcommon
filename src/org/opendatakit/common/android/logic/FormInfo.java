...
public class FormInfo {
	...
	private Map<String, Object> getSetting(String settingName,
			ArrayList<Map<String, Object>> settings) {
	 ...
	 * from that cursor. If parseFormDef is true, read and parse the
	 * formDef.json file.
	 *
	 * @param c
	...
	public FormInfo(Cursor c, boolean parseFormDef) {
		...
		formMediaPath = c.getString(c
				.getColumnIndex(FormsColumns.FORM_MEDIA_PATH));
		formFilePath = c.getString(c
		...
		displaySubtext = c.getString(c
				.getColumnIndex(FormsColumns.DISPLAY_SUBTEXT));
		language = c.getString(c
				.getColumnIndex(FormsColumns.DEFAULT_FORM_LOCALE));
		xmlSubmissionUrl = c.getString(c
				.getColumnIndex(FormsColumns.XML_SUBMISSION_URL));
		xmlBase64RsaPublicKey = c.getString(c
				.getColumnIndex(FormsColumns.XML_BASE64_RSA_PUBLIC_KEY));
		xmlDeviceIdPropertyName = c.getString(c
				.getColumnIndex(FormsColumns.XML_DEVICE_ID_PROPERTY_NAME));
		xmlUserIdPropertyName = c.getString(c
				.getColumnIndex(FormsColumns.XML_USER_ID_PROPERTY_NAME));
		xmlRootElementName = c.getString(c
				.getColumnIndex(FormsColumns.XML_ROOT_ELEMENT_NAME));
	...
	public FormInfo(Context c, File formDefFile) {
		...
		formFilePath = parentFile.getAbsolutePath() + File.separator
				+ parentFile.getName() + ".xml";
		...
		if (settings == null) {
			throw new IllegalArgumentException(
					"File is not a formdef json file! "
							+ formDefFile.getAbsolutePath());
		...
		if (setting != null) {
			...
			throw new IllegalArgumentException(
					"formId is not specified in the formdef json file! "
							+ formDefFile.getAbsolutePath());
		}
		...
		if (setting != null) {
			...
			if (o == null) {
				...
				throw new IllegalArgumentException(
						"defaultLocale is invalid in the formdef json file! "
								+ formDefFile.getAbsolutePath());
			}
		} else {
			defaultLanguage = null;
		...
		if (setting != null) {
			...
			if (o instanceof String) {
				language = (defaultLanguage != null) ? defaultLanguage
						: fallbackLanguage;
				formTitle = (String) o;
			} else {
				try {
					...
					if (!formDefStruct.containsKey(defaultLanguage)) {
						String[] values = formDefStruct.keySet().toArray(
								new String[formDefStruct.size()]);
					...
					e.printStackTrace();
					throw new IllegalArgumentException(
							"formTitle is invalid in the formdef json file! "
									+ formDefFile.getAbsolutePath());
				}
			}
		} else {
		...
		if (setting != null) {
			...
			if (o == null) {
				...
				throw new IllegalArgumentException("Invalid value for "
						+ FORMDEF_XML_USER_ID_PROPERTY_NAME);
			}
		} else {
			xmlUserIdPropertyName = null;
		}
		...
		String ts = new SimpleDateFormat(
				c.getString(R.string.added_on_date_at_time),
