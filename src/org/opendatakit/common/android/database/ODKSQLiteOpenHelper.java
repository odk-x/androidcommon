...
public abstract class ODKSQLiteOpenHelper {
	 ...
	 */
	public ODKSQLiteOpenHelper(String path, String name, CursorFactory factory,
			int version) {
		if (version < 1)
	...
	private synchronized SQLiteDatabase getPrivateWritableDatabase() {
		...
		try {
			...
			if (mName == null) {
				...
				db = SQLiteDatabase.openOrCreateDatabase(mPath + File.separator
						+ mName, mFactory);
